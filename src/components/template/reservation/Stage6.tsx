...
const Stage6 = ({ data, serviceData }: Stage6Props) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6 text-center p-6">
      <h2 className="text-2xl font-bold text-green-600">رزرو با موفقیت انجام شد!</h2>
      ...
      <div className="p-4 border rounded shadow-md max-w-md w-full space-y-4 bg-white">
        ...
        <div className="space-y-1 text-right">
          ...
          <p><strong>قیمت کل:</strong> {data?.total_price?.toLocaleString()} تومان</p>
          <p><strong>مدت زمان رزرو:</strong> {data?.reserve_duration} دقیقه</p>
          <p><strong>توضیحات ادمین:</strong> {data?.admin_description || "—"}</p>
        </div>
      </div>
    </div>
  );
